            ...
            if render:
                if save_images:
                    ...
                    img = env.env.sim.render(
                        width=1280, height=800, camera_name="side_run")
...
def rollout_episodes_AttentionMap(env, agent, num_episodes=1, num_steps=1000, render=True, save_images=None, explore_during_rollout=None, tvel=None):
    """
        Rollout an episode:
        step through an episode, using the 
        - agent = trained policies (is a multiagent consisting of a dict of agents)
