...
exp_path = ["/home/nitro/ray_results/Cheetah_1_1_BipedMultiEnv_Centralized",
            ...
            "/home/nitro/ray_results/Cheetah_1_1_BipedMultiEnv_SixFullyDecentral",
            ...
            "/home/nitro/ray_results/Cheetah_1_1_BipedMultiEnv_SixDecentral_BioGraph",
            ...
            "/home/nitro/ray_results/Cheetah_1_curriculumMass_BipedMultiEnv_TwoSides_GCNCur",
            "/home/nitro/ray_results/Cheetah_1_1_BipedMultiEnv_SixDecentral_AttentionMap", ]
...
all_exp_data = []
for exp_dir in experiment_dirs[:-1]:
...
for i in range(0, len(exp_dir)):
    df = pd.read_csv(exp_dir[i]+'/progress.csv')
    rew_new = (df.iloc[:, 2].values-df.iloc[:, 36].values)
    if i == 0:
        reward_values = np.vstack([rew_new])
        time_steps = (df.iloc[:, 6].values)
    else:
...
for i in range(0, len(all_exp_data)):
    ...
    plt.plot(time_steps, all_exp_data[i][0], color=tableau20[i*2],
             lw=1, label=exp_path[i].split('_')[-1])
