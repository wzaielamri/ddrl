 ...
 #   def healthy_reward(self):
  #      return 0.
    ...
    def step(self, action):
        ...
        forward_reward = (1. + 1./self.target_vel[0]) * (1. / (
            np.abs(x_velocity - self.target_vel[0]) + 1.) - 1. / (self.target_vel[0] + 1.))
        ...
        if done or self.step_counter == self.max_steps:
            ...
            distance = (self.sim.data.qpos[0] - self.start_pos)
    ...
    def _get_obs(self):
        ...
        * last control signal
        ...
        observations = np.concatenate(
            (position, velocity, joint_sensor_forces, last_control, self.target_vel))
        return observations
